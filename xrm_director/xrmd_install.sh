...
confirm_action() {
    ...
    while true; do
        if [[ "$default_value" == "y" ]]; then
            read -p "$prompt (д/н, y/n) [Д/Y]: " response
        else
            read -p "$prompt (д/н, y/n) [Н/N]: " response
        fi
        ...
        # Проверяем ответ (поддерживаем русские и английские варианты)
        case "$response" in
            д|да|y|yes|1)
            ...
            н|нет|n|no|0)
                ...
                ;;
            *)
                echo "Пожалуйста, введите 'д' (да), 'н' (нет), 'y' (yes) или 'n' (no)"
                ;;
        esac
    done
}
...
cli_install() {
    ...
    if install_xrm_director_cli; then
        echo ""
        echo "🎉 Автоматическая установка завершена успешно!"
        echo "🌐 XRM Director доступен по адресу: http://localhost"
        echo "📁 Директория установки: $INSTALL_DIR"
        echo "📋 Логи установки: $LOG_FILE"
        echo ""
        echo "Для управления используйте: sudo $0"
    else
...
# Универсальная функция для запроса подтверждения (поддерживает русские и английские варианты)
ask_yes_no() {
    local prompt="$1"
    ...
    while true; do
        ...
        # Проверяем ответ (поддержка русского и английского)
        case "$answer" in
            [Дд]|[Yy])
                return 0  # Да
                ;;
            [Нн]|[Nn])
                ...
                ;;
            *)
                echo "❌ Пожалуйста, введите: д/y (да) или н/n (нет)"
                ;;
        esac
    done
}
...
remove_xrm_director() {
    ...
    if [ -d "$INSTALL_DIR" ]; then
        ...
        echo "Хотите удалить директорию установки? (д/н)"
        read -r remove_install_dir
        if [[ "$remove_install_dir" =~ ^[Дд]$ ]]; then
            rm -rf "$INSTALL_DIR"
    ...
    if [ -d "$BACKUP_DIR" ]; then
        ...
        echo "Хотите удалить директорию с резервными копиями? (д/н)"
        read -r remove_backup_dir
        if [[ "$remove_backup_dir" =~ ^[Дд]$ ]]; then
            rm -rf "$BACKUP_DIR"
    ...
    echo "Хотите удалить все неиспользуемые Docker образы? (д/н)"
    read -r cleanup_images
    if [[ "$cleanup_images" =~ ^[Дд]$ ]]; then
