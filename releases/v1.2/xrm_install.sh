...
if ! command -v docker &> /dev/null; then
    ...
    if sudo docker ps -a --format '{{.Names}}' | grep -q 'xrm-'; then
        ...
        echo -e "\e[32m1. Создание директории xrm_${ver_path} для X Recovery Manager в каталоге /home\e[0m"
        ...
        sudo wget https://files.x-rm.ru/releases/v1.2/xrm-docker_${ver_path}.tar.gz
